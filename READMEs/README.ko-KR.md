...
- **실패 시 tee-cache** *(신규, rtk에서)* — 공격적인 잘라내기는 복구 가능할 때만 안전합니다:
  실패 시 전체 출력이 `.orchestrator/tee/…log`에 기록되고 경로만 표면에 드러나므로, 에이전트는
  명령을 **다시 실행하지 않고** 컨텍스트를 복구합니다.
...
폴백을 포함한 전체 표는 [`SKILL.md`](../.claude/skills/simplicio-tasks/SKILL.md)의 Step 1b
표에 있습니다.
