...
엔진은 **무료이며 MIT**입니다 — 완전히 셀프 호스팅 가능하고, 결코 기능이 제한되지 않습니다.
제안된 **오픈 코어 호스팅 티어**(관리형 24/7 워처, 호스팅 오퍼레이터, 유지되는 절감 대시보드,
분산 `video_evidence` 렌더)는 [`PRICING.md`](../PRICING.md)에 스케치되어 있으며, 루프가 이미
생성하는 계측 프리미티브(`loop-budget.json` 킬 스위치 + `savings_ledger`)를 바탕으로 한
결정론적·프라이버시 보존형 과금 아키텍처가 함께 제시됩니다. 이는 제안일 뿐이며 — 오늘 청구되는
것은 없습니다.
