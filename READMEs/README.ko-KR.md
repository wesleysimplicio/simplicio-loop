...
실행 가능하며, 테스트되었습니다**(`python3 scripts/check.py`: claims-audit 4/4 + 27 tests). 각
항목은 해당 심층 섹션과 워커로 링크됩니다.

| 역량 | 하는 일 | 증명 / 워커 | 상세 |
|---|---|---|---|
| 🎬 **비디오 증거** (`video_evidence`) | [hyperframes](https://github.com/heygen-com/hyperframes)로 화면/기능의 **결정론적 MP4** 데모를 렌더링 — `/simplicio-tasks make a demo video of screen X`를 충족하며 UI 변경이 동작한다는 CI 재현 가능한 증거 역할도 함 | `scripts/video_evidence.py` · Node 22+/FFmpeg 없으면 BLOCKED(절대 가짜 통과 없음) | [§ 비디오 증거](#-비디오-증거--hyperframes를-통한-데모-비디오) |
...
| 11 | 🎬 **video_evidence (hyperframes)** | [hyperframes](https://github.com/heygen-com/hyperframes) | 화면/기능의 **결정론적 MP4** 데모 비디오를 렌더링 — `/simplicio-tasks make a demo video of screen X`를 충족하며 UI 변경이 동작한다는 CI 재현 가능한 증거 역할도 함 | 증거 생산자 |
