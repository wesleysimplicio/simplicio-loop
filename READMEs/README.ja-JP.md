...
テスト済み**です（`python3 scripts/check.py`：claims-audit 4/4 ＋ 27テスト）。各項目は対応する詳細
セクションとワーカーへのリンクを持ちます。

| 機能 | 何をするか | 証明／ワーカー | 詳細 |
|---|---|---|---|
| 🎬 **Video evidence**（`video_evidence`） | 画面／機能の**決定論的なMP4**デモを [hyperframes](https://github.com/heygen-com/hyperframes) でレンダリング——`/simplicio-tasks make a demo video of screen X` を満たし、UI変更が動作することのCI再現可能な証明も兼ねる | `scripts/video_evidence.py` · Node 22+／FFmpeg がなければ BLOCKED（決してフェイクで通さない） | [§ Video evidence](#-video-evidence--demo-videos-via-hyperframes) |
...
| 11 | 🎬 **video_evidence (hyperframes)** | [hyperframes](https://github.com/heygen-com/hyperframes) | 画面／機能の**決定論的なMP4**デモ動画をレンダリング——`/simplicio-tasks make a demo video of screen X` を満たし、かつUI変更が動作することのCI再現可能な証明も兼ねる | エビデンスのプロデューサー |
