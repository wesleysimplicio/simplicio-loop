...
Движок **бесплатен и под MIT** — полностью самохостимый, никогда не урезанный. Предлагаемый
**open-core хостируемый тариф** (управляемый watcher 24/7, хостируемые операторы, сохраняемая панель
экономии, распределённый рендеринг `video_evidence`) набросан в [`PRICING.md`](../PRICING.md) вместе
с детерминированной, сохраняющей приватность архитектурой биллинга, построенной на примитивах
метрики, которые цикл уже производит (kill-switch `loop-budget.json` + `savings_ledger`). Это
предложение — сегодня ничего не выставляется к оплате.
