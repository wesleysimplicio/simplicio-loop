...
def run_installed_smoke(repo: str, probes: Mapping[str, Callable[[Mapping[str, Any]], Mapping[str, Any]]], *,
                        mapper_envelope_hash: str = "", plan_hash: str = "") -> dict[str, Any]:
    ...
    context = {"schema": SCHEMA, "repo": repo, "correlation_id": correlation_id,
               "mapper_envelope_hash": mapper_envelope_hash, "plan_hash": plan_hash}
    ...
    for name in COMPONENTS:
        ...
        if result["status"] != "READY":
            break
    ready = all(components.get(name, {}).get("status") == "READY" for name in COMPONENTS)
    return {
        "schema": SCHEMA, "status": "READY" if ready else "BLOCKED",
        "installed": True, "effects_attempted": False, "correlation_id": correlation_id,
        "mapper_envelope_hash": mapper_envelope_hash, "plan_hash": plan_hash,
        "components": components, "report_hash": _digest(components),
    }


__all__ = ["COMPONENTS", "InstalledE2EError", "SCHEMA", "run_installed_smoke"]
