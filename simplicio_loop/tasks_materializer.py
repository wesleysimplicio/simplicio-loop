...
class LoopRunContractMaterializer:
    ...
    def __call__(self, intake: Mapping[str, Any]) -> list[Mapping[str, Any]]:
        ...
        for number, item in sorted((intake.get("items") or {}).items(), key=lambda row: str(row[0])):
            ...
            rows.append({"repo": str(self.repo), "run_id": armed["manifest"]["run_id"], "task_index": 1, "task_id": f"issue-{number}", "isolation": "worktree", "isolation_key": f"issue-{number}", "task_spec": {"id": f"issue-{number}", "goal": str(item.get("title") or ""), "files_affected": targets}})
        return rows
