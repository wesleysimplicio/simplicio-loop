#!/usr/bin/env python3
"""Real worker-daemon lifecycle supervisor for issue #286.

Runs as its own OS process. Spawns ``--workers`` copies of
``scripts/remote_worker_daemon.py serve`` (real ``subprocess.Popen`` children, not threads),
polls each child's liveness every ``--health-interval`` seconds, and restarts (respawns) any
child that has exited -- crashed, killed, or otherwise -- after ``--restart-backoff-seconds``.
This is what makes "worker daemon lifecycle/supervisor" real rather than prose: a `kill -9`
(POSIX) / ``TerminateProcess`` (Windows) on a managed worker is detected and recovered from
without any manual intervention, proven by ``tests/test_remote_worker_supervisor.py`` killing a
real child process and observing a genuinely new PID take its place.
...
_DAEMON = _REPO_ROOT / "scripts" / "remote_worker_daemon.py"
...
class ManagedWorker:
    """One supervised ``remote_worker_daemon.py serve`` child process."""

    def __init__(self, index: int, argv: List[str], *, status_file: str) -> None:
        self.index = index
        self.argv = argv
    ...
    def _spawn(self) -> subprocess.Popen:
        return subprocess.Popen(
            [sys.executable, str(_DAEMON), *self.argv],
...
def build_supervisor(args: argparse.Namespace) -> WorkerSupervisor:
    ...
    for i in range(args.workers):
        ...
        workers.append(ManagedWorker(i, argv, status_file=status_file))
