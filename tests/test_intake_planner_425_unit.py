...
def test_single_task_fast_cli_is_real_fail_closed_dispatch(tmp_path, capsys):
    ...
    assert payload["reason_code"] in {"optional_local_operator_unavailable", "mapper_operation_failed", "local_operation_failed"}
...
def test_mapper_foreground_accepts_hashed_standalone_handoff(tmp_path, monkeypatch):
    from simplicio_loop.intake_planner import build_local_single_task_operations, content_hash
    import subprocess

    handoff = {
        "schema": "simplicio.mapper-fast-handoff/v1",
        "canonical_map": {"generation": "M1"},
    }
    receipt = {
        "schema": "simplicio.mapper-fast-handoff-receipt/v1",
        "status": "parsed",
        "generation": "M1",
        "handoff_sha256": content_hash(handoff),
    }
    payload = {"handoff": handoff, "receipt": receipt}
    observed = []
    ...
    operations = build_local_single_task_operations(_fast_task(repo=str(tmp_path)), root=str(tmp_path))
    foreground = operations["mapper_foreground"]({})

    assert foreground["verified"] is True
    assert foreground["generation"] == "M1"
    assert observed[0][1] == "fast-handoff"
